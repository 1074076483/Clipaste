...
final class MigrationManager: ObservableObject {
    enum MigrationSource: String, CaseIterable, Identifiable {
        ...
        nonisolated var guidanceText: String {
            switch self {
            case .paste:
                "请选择 \(displayName) 的 Core Data SQLite 数据库，通常位于 ~/Library/Containers/.../Data/Library/Application Support/。"
            case .pasteNow:
                "请选择 \(displayName) 导出的 JSON 文件。Clipaste 会按 PasteNow 专用 JSON 路由解析。"
            case .iCopy:
                "请选择 \(displayName) 的 SQLite 数据库，通常位于 ~/Library/Containers/cn.better365.iCopy/... 目录下。"
            }
        }
    ...
    private func importRows(
        ...
        return MigrationReport(importedCount: importedCount, skippedCount: skippedCount)
    }
}
...
private extension MigrationManager {
    ...
    nonisolated static func readPasteSQLiteRows(from fileURL: URL) throws -> [MigratedClipboardRow] {
        ...
        // Phase 3: Hardcoded query for Paste's ZITEMENTITY table
        let sql = "SELECT ZRAWPREVIEW FROM ZITEMENTITY WHERE ZRAWPREVIEW IS NOT NULL;"
...
private extension MigrationManager {
    ...
    nonisolated static func readICopySQLiteRows(from fileURL: URL) throws -> [MigratedClipboardRow] {
        ...
        // Phase 4: Hardcoded query for iCopy's t_data table
        let sql = "SELECT text FROM t_data WHERE text IS NOT NULL;"
        var statement: OpaquePointer?
        let prepareCode = sqlite3_prepare_v2(database, sql, -1, &statement, nil)
    ...
    nonisolated static func collectJSONRows(
        ...
        case let array as [Any]:
            for element in array {
                collectJSONRows(from: element, configuration: configuration, into: &rows)
            }

        case let dictionary as [String: Any]:
            if let row = makeJSONRow(from: dictionary, configuration: configuration) {
                rows.append(row)
            }
            ...
            for (key, value) in dictionary {
                ...
                collectJSONRows(from: value, configuration: configuration, into: &rows)
            }
